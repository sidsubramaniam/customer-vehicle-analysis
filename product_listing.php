...
$prod_images=$_FILES["prod_images"]['name'];
$prod_active_status;
...
if($conn->connect_error){
    ...
    $stmt=$conn->prepare("insert into prod(user_id,cat_id,prod_name,prod_desc,prod_active_status)values(?,?,?,?,1)");
    $stmt->bind_param("iiss",$user_id,$cat_id,$prod_name,$prod_desc);
    $stmt->execute();

    
    echo "<br> PRODUCT REGISTERED <br><br>";
    echo "<hr><br>";
...
for($i=0;$i<$fileCount;$i++){
    ...
    $fileLoc='http://localhost/internship/responsive%20registration%20form/prod_images/'.$fileName;
